const connection = require("../config/db");
    ...
    class ActivityController {
      createActivity = (req, res) => {
        const { date_time_activity, limit_users, text, activity_city, details, sport_id, user_id, maps_link } = req.body;
        ...
        if (limit_users !== null && limit_users <= 0) {
          return res.status(400).json({ error: "El límite de usuarios debe ser un número positivo." });
        }
        ...
        connection.query(sqlCheckDuplicate, [date_time_activity, activity_city, sport_id], (err, result) => {
          ...
          const values = [date_time_activity, limit_users || null, text, activity_city, details, user_id, sport_id, maps_link || null];
